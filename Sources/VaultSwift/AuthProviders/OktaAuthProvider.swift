...
struct OktaAuthProvider: AuthProvider {
    ...
    func getToken() async throws(VaultError) -> String {
        let response: VaultResponse<[String: JSONAny]> = try await client.makeCall(path: "/auth/\(mount?.trim() ?? "okta")/login/\(request.username)", httpMethod: .post, request: request, wrapTimeToLive: nil)
