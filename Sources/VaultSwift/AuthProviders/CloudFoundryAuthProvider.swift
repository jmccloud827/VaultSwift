...
struct CloudFoundryAuthProvider: AuthProvider {
    ...
    func getToken() async throws(VaultError) -> String {
        let response: VaultResponse<[String: JSONAny]> = try await client.makeCall(path: "v1/auth/\(mount?.trim() ?? "cf")/login", httpMethod: .post, request: request, wrapTimeToLive: nil)
