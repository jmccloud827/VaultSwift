...
extension Vault.AuthProviders {
    struct UserCredentialsTokenProvider: TokenProvider {
        ...
        func getToken() async throws(VaultError) -> String {
            ...
            let response: VaultResponse<[String: JSONAny]> = try await client.makeCall(path: "/auth/\(mount?.trim() ?? MethodType.userCredentials.rawValue)/login/\(username)", httpMethod: .post, request: request, wrapTimeToLive: nil)
