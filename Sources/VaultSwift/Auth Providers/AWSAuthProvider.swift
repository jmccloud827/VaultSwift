...
struct AWSAuthProvider: AuthProvider {
    let role: String
    let nonce: String
    ...
    func getToken() async throws(VaultError) -> String {
        ...
        let response: VaultResponse<[String: JSONAny]> = try await client.makeCall(path: "v1/auth/\(mount?.trim() ?? AuthProviderType.aws.rawValue)/login", httpMethod: .post, request: request, wrapTimeToLive: nil)
