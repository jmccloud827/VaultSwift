...
struct CloudFoundryAuthProvider: AuthProvider {
    let request: CloudFoundryAuthRequest
    let mount: String?
    let client: Vault.Client
        
    func getToken() async throws(VaultError) -> String {
        let response: VaultResponse<[String: JSONAny]> = try await client.makeCall(path: "v1/auth/\(mount?.trim() ?? AuthProviderType.cloudFoundry.rawValue)/login", httpMethod: .post, request: request, wrapTimeToLive: nil)
