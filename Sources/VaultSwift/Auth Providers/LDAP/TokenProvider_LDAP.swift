...
extension Vault.AuthProviders {
    struct LDAPTokenProvider: TokenProvider {
        ...
        func getToken() async throws -> String {
            ...
            let response: VaultResponse<JSONAny> = try await client.makeCall(path: "/auth/\(mount?.trim() ?? MethodType.ldap.rawValue)/login/\(username)", httpMethod: .post, request: request, wrapTimeToLive: nil)
