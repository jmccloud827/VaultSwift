...
extension Vault.AuthProviders {
    struct GoogleCloudTokenProvider: TokenProvider {
        ...
        func getToken() async throws(VaultError) -> String {
            let response: VaultResponse<[String: JSONAny]> = try await client.makeCall(path: "v1/auth/\(mount?.trim() ?? MethodType.googleCloud.rawValue)/login", httpMethod: .post, request: request, wrapTimeToLive: nil)
