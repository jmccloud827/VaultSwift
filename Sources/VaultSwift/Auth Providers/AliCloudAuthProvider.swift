...
struct AliCloudAuthProvider: AuthProvider {
    ...
    func getToken() async throws(VaultError) -> String {
        let response: VaultResponse<[String: JSONAny]> = try await client.makeCall(path: "v1/auth/\(mount?.trim() ?? AuthProviderType.aliCloud.rawValue)/login", httpMethod: .post, request: request, wrapTimeToLive: nil)
