...
struct OCIAuthProvider: AuthProvider {
    ...
    func getToken() async throws(VaultError) -> String {
        ...
        let response: VaultResponse<[String: JSONAny]> = try await client.makeCall(path: "/auth/\(mount?.trim() ?? AuthProviderType.oci.rawValue)/login/\(role)", httpMethod: .post, request: request, wrapTimeToLive: nil)
